...
func _ready():
	...
	for i in range(2):
		...
		player.bus = "Music"
		player.finished.connect(_on_song_finished.bind([player]))
...
func play_song(song_name: String, transition: float = 2.0):
	...
	idle_player.volume_db = linear_to_db(0) # begin with 0 and fade to volume
	idle_player.play()
...
#	tween.interpolate_method(self, "set_active_player_volume", db_to_linear(active_player.volume_db), 0, transition)
#	tween.interpolate_method(self, "set_idle_player_volume", db_to_linear(idle_player.volume_db), db2linear(song["volume"]), transition)
	active_player.stop()
	idle_player.volume_db = db_to_linear(song["volume"])
...
func set_active_player_volume(new_volume: float):
	active_player.volume_db = linear_to_db(new_volume)


func set_idle_player_volume(new_volume: float):
	idle_player.volume_db = linear_to_db(new_volume)
